...
require(['global', "core", "xqvalidate"], function(g, core, xqvalidate) {
	var jgid = "";
	...
	if (params.isPass != null || params.isPass != undefined) {
		...
		if (params.isPass == 1) {
			...
			alertInfo(decodeURI(params.msg));
		} else {
			alertInfo(decodeURI(params.msg));
		}
	}
	...
	var baoMingModel = function() {
			...
			core.getJSONPCache(url, {
				...
				core.result(data, function(data) {
					...
					var baoMing = $("#baoming"),
						baoMingXiangQing = '<p>{JXNAME}<span>{BXMC}</span></p><p>￥{YHJG}</p>',
...
//		var index = url.indexOf("login=1");
	...
	$('#btn_commit').on('click', function() {
		if ($('#ClientCode').val() != 3) {
			alertInfo('出现错误，请刷新重试');
			return;
		}
		if ($('#BXID').val() == null || $('#BXID').val() == undefined) {
			alertInfo('对不起，班型出现错误，请稍后重试');
			return;
		}
		...
		var isPass = false;
		isPass = validateFunction.FORM_submit(['#textName', '#textTel']);

		if (isPass) {
			if (!$('#xieyi').is(":checked")) {
				alertInfo('请阅读并同意《三方代收代付协议》');
				return;
			}
//			g.urlHttp = 'http://testxcbapi.xuechebu.com/';
			$('#fm').attr('action', g.urlHttp + 'JXBMX/BaoMingByMinShengPhone');
...
//			}
			...
			function SubFunc(){
				var cfg = {
					...
					'ok_btn' : 'true', // 显示ok按钮
					'ok_text' : '确定', // ok按钮的内容
					'ok_func' : 'submintBaoming()', // 点击ok按钮调用的方法
					'cancle_text' : '返回', // 显示取现按钮
					'cancle_func' : ''  // 点击取现按钮,客户端回调服务器的方法
				};
				confirmInfo (cfg);
			}
			SubFunc();
