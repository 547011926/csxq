...
var validateRules = {
	...
	isMoney: function (str) {
		var b=new RegExp(/^([1-9][\d]{0,7}|0)(\.[\d]{1,2})?$/).test(str);
        return b;
    },
	isEmail: function (str) {
        return new RegExp(/[\w!#$%&'*+/=?^_`{|}~-]+(?:\.[\w!#$%&'*+/=?^_`{|}~-]+)*@(?:[\w](?:[\w-]*[\w])?\.)+[\w](?:[\w-]*[\w])?/).test(str);
   },
...
var validateFunction = {
	...
	checkSelectGroup: function (option) {
	...
	sfzh: function (option) {
        option.value = option.value;
        var f = validateRules.isNull(option.value);
        if (f) {
            validateSettings.error.run(option, option.prompts.isNull);
            $("#xx_zjhm_error").addClass("grdd");
            return;
        }
        else {
            var bool2 = validateRules.isSfzh(option.value);
            if (!bool2) {
                validateSettings.error.run(option, option.prompts.error);
                $("#xx_zjhm_error").addClass("grdd");
                return;
...
//		}
	...
	email: function (option) {
        ...
        if (f) {
            validateSettings.error.run(option, option.prompts.isNull);
            return;
        } else if (!validateRules.isEmail(option.value)) {
            validateSettings.error.run(option, option.prompts.error);
            return;
        }
        else {
    ...
    mobileCode: function (option) {
        ...
        if (f) {
            validateSettings.error.run(option, option.prompts.isNull);
            return;
        } else if (!validateRules.isMobile(option.value)) {
            validateSettings.error.run(option, option.prompts.error);
            return;
        }
        else {
    ...
    money: function (option) {
        var f = validateRules.isNull(option.value);
        if (f) {
            validateSettings.error.run(option, option.prompts.isNull);
            return;
        } else if (!validateRules.isMoney(option.value)) {
            validateSettings.error.run(option, option.prompts.error);
            return;
        }
        else {
