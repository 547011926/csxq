...
require(['global', "core","user"], function (g, core,u) {
    ...
    var getBXJAList = function () {
        ...
        core.getJSONCache(url, {}, function (data) {
            ...
            var banXingJiaGeList = '<div class="banxingjiage-list"><div class="drivingImg">{BXIMG}</div><div class="driving"><h1>{BXMC}</h1><h2>￥{YHJG}<span>{JG}</span></h2><p>{LCSJ}</p><p>{CXMC}&nbsp;&nbsp;{JZLX}</p><a href = "baominginfo.html?'+
                (u.checklogin()? '' : 'login=1&gonext=no&') +'id={ID}">立即报名</a></div></div> ';
            var ary=[];
